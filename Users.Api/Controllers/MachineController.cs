...
{
    // Validar que se haya especificado un WorkStationId válido
    if (createDto.WorkStationId <= 0)
        return BadRequest("WorkStationId debe ser mayor que 0.");

    // Verificar que el WorkStation exista
    var exists = await _context.WorkStations.AnyAsync(w => w.WorkStationId == createDto.WorkStationId);
    if (!exists)
        return BadRequest("El WorkStationId especificado no existe.");

    // Mapear DTO a entidad Machine
    ...
    // Retornar la respuesta con ubicación del recurso creado
    return CreatedAtAction(nameof(GetMachine), new { id = machine.MachineId }, readDto);
}
